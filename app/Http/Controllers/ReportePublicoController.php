...
class ReportePublicoController extends Controller {
    ...
    public function listaReportes(Request $request) {
        ...
        $palabra_clave_id = $request->get('palabra_clave_id');
        $ponderacion_id = $request->get('ponderacion_id');
        ...
        if (($institucion_id != -1) && ($institucion_id != '')) {
            $i = " and i.id in ($institucion_id) ";
        }
        if (($consejoSectorial_id != -1) && ($consejoSectorial_id != '')) {
            $cs = " and  cs.id in ($consejoSectorial_id) ";
        }
        if (($estado_id != -1) && ($estado_id != '')) {
            $e = "  and e.id in ($estado_id) ";
        }
        if (($zona_id != -1) && ($zona_id != '')) {
            $z = " and  z.id in ($zona_id) ";
        }
        if (($mesaDialogo_id != -1) && ($mesaDialogo_id != '')) {
            $m = " and  m.id in ($mesaDialogo_id) ";
        }
        if (($palabra_clave_id != '-1') && ($palabra_clave_id != '')) {
        ...
        $palabraClave_ = DB::select("SELECT nombre FROM palabras_claves group by nombre");
